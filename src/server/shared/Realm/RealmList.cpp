...
void RealmList::UpdateRealms(boost::system::error_code const& error)
{
    ...
    TC_LOG_DEBUG("server.authserver", "Actualizando Lista de Reinos..");
        ...
        do
        {
            try
            {
                ...
                if (!existingRealms.count(id))
                    TC_LOG_INFO("server.authserver", "Añadido el Reino \"%s\" at %s:%u.", name.c_str(), externalAddressString.c_str(), port);
                else
