...
extern int main(int argc, char** argv)
{
    ...
    Trinity::Banner::Show("worldserver-daemon",
        ...
        []()
        {
            TC_LOG_INFO("server.worldserver", "Usando el archivo de configuracion %s.", sConfigMgr->GetFilename().c_str());
            TC_LOG_INFO("server.worldserver", "Usando la version de SSL: %s (library: %s)", OPENSSL_VERSION_TEXT, SSLeay_version(SSLEAY_VERSION));
            TC_LOG_INFO("server.worldserver", "Usando la version de Boost: %i.%i.%i", BOOST_VERSION / 100000, BOOST_VERSION / 100 % 1000, BOOST_VERSION % 100);
        }
    );
    ...
    TC_LOG_INFO("server.worldserver", "%s (worldserver-daemon) listo...", GitRevision::GetFullVersion());
    ...
    TC_LOG_INFO("server.worldserver", "Deteniendo proceso...");
...
bool StartDB()
{
    ...
    TC_LOG_INFO("server.worldserver", "Reino ID actualmente corriendo %d", realm.Id.Realm);
    ...
    TC_LOG_INFO("server.worldserver", "DB usada para Mundo: %s", sWorld->GetDBVersion());
    return true;
}
